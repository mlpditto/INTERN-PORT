(function(){
 ...
 function render(){
  ...
  const grid=el('div');grid.className='mp-grid';let missing=false;
  [['quiz','📚 Quiz',count(quizzes,true)],['log','📝 Journal',count(data.logs)],['case','🩺 Case',count(data.cases)]].forEach(([key,label,n])=>{const card=el('div');card.className='mp-card';card.title=key==='log'?'บันทึกประจำวันและ Learning Notes ที่ส่งในเดือนนี้':'จำนวนที่ส่งในเดือนนี้';card.append(el('strong',label));const target=data.targets?.[key];if(!target)missing=true;const value=el('div',n===null?'—':n.toLocaleString());value.className='mp-value';card.append(value,el('small',n===null?'Loading…':target?'of '+target+' · '+Math.round(n/target*100)+'%':'submitted'));if(target&&n!==null){const bar=el('progress');bar.max=target;bar.value=Math.min(n,target);bar.setAttribute('aria-label',label+' progress');bar.title='ส่งในเดือนนี้ '+n+' จากเป้าหมาย '+target;card.append(bar);}grid.append(card);});host.append(grid);
  if(missing){const hint=el('div');hint.className='mp-target-hint';hint.append(el('span','🎯 Set monthly targets to see your progress'));const set=el('button','Set targets →');set.title='ตั้งเป้ารายเดือน';set.onclick=edit;hint.append(set);host.append(hint);}
 }
 function edit(){
  if(!data)return;
