...
const dir='docs/ai-models/phase3',rows=[];
async function main(){
 const env={};
 ...
 const models=registry.models.filter(m=>m.selectable);
 for(let round=1;round<=3;round++){
  const order=[...models.slice(round-1),...models.slice(0,round-1)];
  const jobs=order.flatMap(model=>['audit','translation','generation'].map(task=>({model,task})));
  let index=0;
  async function worker(){while(index<jobs.length){const {model,task}=jobs[index++],started=Date.now();
   ...
   try{
    const prompt=fs.readFileSync(`${dir}/${task}.txt`,'utf8');let result;
    ...
    }else result=await runModernAI({model:model.id,prompt,isJson:true,generationOptions:{maxOutputTokens:8192}},post,env);
   ...
   row.latencyMs=Date.now()-started;rows.push(row);fs.writeFileSync(`${dir}/results.json`,JSON.stringify(rows,null,2)+'\n');console.log(JSON.stringify({...row,result:undefined,usage:undefined}));
  }}
