...
async function run({bonus=0,awards=[],changed=false,owner='user',gone=false,reordered=false}={}){
 ...
 const db={collection:c=>({doc:id=>ref(c,id||'log'),where:()=>({get:async()=>({docs:awards.map(amount=>({data:()=>({type:'learning_note_bonus',amount})}))})})}),runTransaction:async fn=>{const staged=[];const tx={get:async r=>r.path==='submissions/item'?{exists:!gone,data:()=>changed?{...note,adminBonus:9}:reordered?{metadata:{sourceId:'source',sourceType:'learning_path_entries'},adminBonus:bonus,score:.1,userId:'user',submissionType:'learning_note'}:note}:{exists:true,data:()=>({userId:owner})},update:(r,d)=>staged.push(['update',r.path,d]),set:(r,d)=>staged.push(['set',r.path,d]),delete:r=>staged.push(['delete',r.path])};const result=await fn(tx);writes=staged;return result}};
...
(async()=>{
 ...
 r=await run({reordered:true});assert.equal(r.alerts.length,0,'same field values in different order must permit deletion');
