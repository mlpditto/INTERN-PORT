...
await page.evaluate(()=>{window.loads=0;const date=new Date().toLocaleDateString('en-CA',{timeZone:'Asia/Bangkok'});window.docs=[{date,totalTokens:1000,totalCount:4,providers:{openai:{tokens:1000,count:4}},models:{a:{model:'Test model',provider:'openai',tokens:800,count:3,inputTokens:500,outputTokens:300,detailedCount:3}}}];window.db={collection:()=>({where:()=>({orderBy:()=>({get:async()=>{loads++;return {docs:docs.map(d=>({data:()=>d}))};}})})})};});
...
await page.locator('#settings .au-scroll button').last().click();assert.match(await page.locator('#settings [role=status]').innerText(),/3 calls/);
await page.evaluate(()=>{docs[0].models={};});await page.locator('#settings').getByRole('button',{name:'Refresh',exact:true}).click();await page.waitForFunction(()=>document.getElementById('settings').textContent.includes('Provider view'));
assert.match(await page.locator('#settings').innerText(),/Unavailable/);
