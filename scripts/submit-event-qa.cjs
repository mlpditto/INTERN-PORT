...
// invite-ticket.js: new invites start private ("Visible to you and Admin") — the admin picks the audience later.
const ctx={window:{getInviteAudience:()=>[]},document:{getElementById:id=>fields[id]},evIsUrl:v=>/^https?:\/\//.test(v),firebase:{auth:()=>({currentUser:{uid:'auth-user'}}),firestore:{FieldValue:{serverTimestamp:()=> 'server-time'}}},userId:'line-user',userProfile:{displayName:'Example',pictureUrl:''},myGroup:'EXTERN',db:{collection:name=>({add:async payload=>{assert.equal(name,'events');if(fail)throw Error('offline');writes.push(payload);}})}};
...
(async()=>{
 for(const prefix of ['sne','ue']){
  ...
  const p=writes.at(-1);assert.equal(p.kind,'social');assert.equal(p.title,'Test event');assert.equal(p.createdByUid,'auth-user');assert.deepEqual(p.targetGroups,[]);assert.equal(p.timeText,'15:00–16:00');assert.equal(p.joinVia,'https://example.com/join');assert.equal(p.location,'');
